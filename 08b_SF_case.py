...
print(f"[{pkinds[pkind]}]: {str1}+{str2}: {len(ids)} founds")
if(len(ids)==0): exit()
...
for targetid in ids:
    ...
    print(f"(Ncpu={ncpu}) drawmode={drawmode}: {imin}~{imax}")

    for iout in tqdm(branch['timestep'][::-1]):
        ...
        isnap.set_box_halo(target, zoom*8)
        ...
        if(len(part['star'])<(nmax/10))and(len(part['star'])>0):
            ax_star.scatter(part['star']['x'], part['star']['y'], s=(nmax/10)/len(part['star']), ec='none', fc='orange',marker="*")
        ...
        ax_cold.text(0.5,0.01, "Dense Gas", color='dodgerblue', ha='center', va='bottom', fontsize=15, transform=ax_cold.transAxes, family='DejaVu Serif', path_effects=[patheffects.withSimplePatchShadow(offset=(0.5,-0.5))])
        ...
        r1 = 34*isnap.unit['pc']; r2 = zoom*target['rvir']
        r1 = max(r1, np.min(cell['dx']))
        bins = np.logspace(np.log10(r1), np.log10(r2), 20+1)
        xs = 0.5*(bins[:-1]+bins[1:])/isnap.unit['kpc'] # kpc
        sax1 = fig.add_subplot(gs[0,3]); sax1.yaxis.tick_right()
        sax1.grid(True, zorder=0, color='lightgray'); sax1.set_axisbelow(True)
        # Member Particles
        ys = np.zeros(len(xs))
        member = uhmi.HaloMaker.read_member_part(isnap, target['id'], galaxy=False, target_fields=['id','x','y','z','m'])
        dist = distance(member, target)
        for i in range(len(xs)):
            ind = (dist > bins[i]) & (dist < bins[i+1])
            vol = (4/3*np.pi*(bins[i+1]**3-bins[i]**3)) # code**3
            vol /= isnap.unit['kpc']**3
            ys[i] = np.sum(member[ind]['m','Msol']) / vol # Msol/kpc3
        ind = ys>0
        sax1.plot(xs[ind], ys[ind], color='darkgray', lw=1.5, ls='-')
        # All Particles
        ys = np.zeros(len(xs))
        ws = np.zeros(len(xs))
        dist = distance(dm, target)
        for i in range(len(xs)):
            ind = (dist > bins[i]) & (dist < bins[i+1])
            vol = (4/3*np.pi*(bins[i+1]**3-bins[i]**3)) # code**3
            vol /= isnap.unit['kpc']**3
            ys[i] = np.sum(dm[ind]['m','Msol']) / vol # Msol/kpc3
            ws[i] = 1 / np.sqrt(np.sum(dm[ind]['m']))
        ind = ys>0
        ws /= np.sum(ws)
        sax1.plot(xs[ind], ys[ind], color='dimgrey', lw=1.5, ls=':')
        ...
        sax1.axvline(target['rvir']/isnap.unit['kpc'], color='dimgrey', linestyle='--', lw=1.5)
        sax1.text(0.05, 0.15, fr"$\gamma={popt[1]:.3f}$", ha='left', va='bottom', fontsize=12, transform=sax1.transAxes, color='royalblue')
        sax1.text(0.05, 0.05, fr"$R_{{vir}}={target['rvir']/isnap.unit['kpc']:.2f}$ [kpc]", ha='left', va='bottom', fontsize=12, transform=sax1.transAxes)
        ...
        if(not os.path.isdir(f"{database1}/photo/evolution/{pkind}/{ptype}/{targetid:07d}")): os.makedirs(f"{database1}/photo/evolution/{pkind}/{ptype}/{targetid:07d}")
        path = f"{database1}/photo/evolution/{pkind}/{ptype}/{targetid:07d}"
        plt.savefig(f"{path}/{targetid:07d}_{iout:04d}.png", dpi=300, facecolor='k', bbox_inches='tight', pad_inches=0.1)
        plt.close()
        isnap.clear()
