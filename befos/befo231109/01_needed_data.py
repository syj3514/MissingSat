...
for key in LG.keys():
    ...
    if(not os.path.exists(f"./database/photo/01_compare_pairs/NH_{key:04d}_mratio.png")):
        ...
        ax.set_aspect(1/2.5)
        fig.savefig(f"./database/photo/01_compare_pairs/NH_{key:04d}_mratio.png", dpi=400, facecolor='white')
        print(f"`./database/photo/01_compare_pairs/NH_{key:04d}_mratio.png` save done")
        plt.close()

mratios = np.rec.fromarrays((hosts, subss['id'],isdinkss, mdms, mstars, mcolds, mcells), names=('host','sub','dink','mdm','mstar','mcold','mcell'))
pklsave(mratios, f"./database/01_mass_in_sub.pickle", overwrite=True)