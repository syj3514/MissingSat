...
for key in LG.keys():
    ...
    dinkss = dinks if(dinkss is None) else np.hstack((dinkss, dinks))
    pairss = pairs if(pairss is None) else np.hstack((pairss, pairs))
    ...
    if(not os.path.exists(f"./database/photo/14_compare_pairs/NH_{key:04d}_mratio.png")):
        ...
        for i,sub in enumerate(subs):
            ...
            ax.plot([np.log10(mdm[i]),np.log10(mdm[i])], [np.log10(mstar[i]), np.log10(mcell[i])], color=linecolor, lw=0.1)
        ...
        ax.set_aspect(1/2.5)
        fig.savefig(f"./database/photo/14_compare_pairs/NH_{key:04d}_mratio.png", dpi=400, facecolor='white')
        print(f"`./database/photo/14_compare_pairs/NH_{key:04d}_mratio.png` save done")
        plt.close()


#<function numpy.rec.fromarrays
# (arrayList, dtype=None, shape=None, formats=None, names=None, titles=None, aligned=False, byteorder=None)>
mratios = np.rec.fromarrays((hosts, subss['id'],isdinkss, mdms, mstars, mcolds, mcells), names=('host','sub','dink','mdm','mstar','mcold','mcell'))
pklsave(mratios, f"./database/14_mass_in_sub.pickle")
...
fig, ax = plt.subplots(dpi=300, figsize=(8,6))
for i,sub in enumerate(subss):
    isdink = sub['id'] in dinkss['hid']
    color = 'k' if(mcolds[i]<=1e3) else 'deepskyblue'
    facecolor='none' if(isdink) else color
    edgecolor=color if(isdink) else 'none'
    linecolor=color

    ax.scatter(np.log10(mdms[i]), np.log10(mstars[i]), facecolor=facecolor, edgecolor=edgecolor, s=50, lw=0.1, marker='*', alpha=0.5)
    ax.scatter(np.log10(mdms[i]), np.log10(mcolds[i]), facecolor=facecolor, edgecolor=edgecolor, s=50, lw=0.1, marker='H', alpha=0.5)
    ax.scatter(np.log10(mdms[i]), np.log10(mcells[i]), facecolor=facecolor, edgecolor=edgecolor, s=50, lw=0.1, alpha=0.5)
    # ax.text(np.log10(mdms[i]), np.log10(mcells[i])+0.1,f"{sub['id']}", ha='center', va='bottom', fontsize=5)
    ax.plot([np.log10(mdms[i]),np.log10(mdms[i])], [np.log10(mstars[i]), np.log10(mcells[i])], color=linecolor, lw=0.1, alpha=0.5)
...
hascold = mcolds > 1e3
haspair = isin(subss['id'], pairss['hid'])
ax.text(0.03, 0.95, fr"$\mathtt{{ N_{{subh}}: {len(subss)} }}$", ha='left', va='top', color='k', fontsize=9, transform=ax.transAxes)
ax.text(0.03, 0.90, fr"$\mathtt{{ N_{{dink}}: {len(dinkss)} }}$", ha='left', va='top', color='k', fontsize=9, transform=ax.transAxes)
ax.text(0.03, 0.85, fr"$\mathtt{{ N_{{pair}}: {len(pairss)} }}$", ha='left', va='top', color='k', fontsize=9, transform=ax.transAxes)
ax.text(0.03, 0.80, fr"$\mathtt{{ N_{{has cold}}: {np.sum(hascold)} }}$", ha='left', va='top', color='deepskyblue', fontsize=9, transform=ax.transAxes)
ax.text(0.25, 0.95, fr"$\mathtt{{ P(pair|cold): {100*np.sum(haspair&hascold)/np.sum(hascold):.1f}\% }}$", ha='left', va='top', color='k', fontsize=11, transform=ax.transAxes)
ax.text(0.25, 0.90, fr"$\mathtt{{ P(cold|pair): {100*np.sum(haspair&hascold)/np.sum(haspair):.1f}\% }}$", ha='left', va='top', color='k', fontsize=11, transform=ax.transAxes)
...
ax.legend(handles=legend_elements, loc='center left', bbox_to_anchor=(1, 0.5), fontsize=8)
ax.set_aspect(1/2.5)
fig.savefig(f"./database/photo/14_compare_pairs/NH_all_mratio.png", dpi=400, facecolor='white')