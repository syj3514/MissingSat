...
for key in tqdm( LG.keys() ):
    ...
    for ireal in real:
        if(ireal['state'])=='pair':
            ...
            cir = plt.Circle((isub['x'], isub['y']), isub['rvir'], color='tomato', fill=False, lw=0.5, ls='-', zorder=10)
            ax.add_patch(cir)
        elif(ireal['state'])=='upair':
            isat = sats[sats['id']==ireal['gid']][0]
            cir = plt.Circle((isat['x'], isat['y']), isat['r'], color='yellow', fill=False, lw=0.5, ls='-', zorder=10)
            ax.add_patch(cir)
            isub = subs[subs['id']==ireal['hid']][0]
            cir = plt.Circle((isub['x'], isub['y']), isub['rvir'], color='tomato', fill=False, lw=0.5, ls='-', zorder=10)
            ax.add_patch(cir)
            ...
            isat = sats[sats['id']==ireal['gid']][0]
            cir = plt.Circle((isat['x'], isat['y']), isat['r'], color='cyan', fill=False, lw=0.25, ls='--', zorder=10)
            ax.add_patch(cir)
        elif(ireal['state'])=='dink':
            isub = subs[subs['id']==ireal['hid']][0]
            cir = plt.Circle((isub['x'], isub['y']), isub['rvir'], color='tomato', fill=False, lw=0.25, ls='--', zorder=10)
            ax.add_patch(cir)
    ...
    # plt.savefig("test.png", dpi=400, facecolor='none', bbox_inches='tight', pad_inches=0)
    plt.savefig(f"{database}/photo/00_LG_image/NH_{key:04d}_all.png", dpi=400, transparent=False, facecolor='white', bbox_inches='tight', pad_inches=0)
    plt.close()