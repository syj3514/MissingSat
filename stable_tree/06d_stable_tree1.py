...
if(not os.path.exists(f"{database1}/stable_tree.pickle")):
    stable_tree1 = {}
...
if(not os.path.exists(f"{database1}/stable_tree_raw.pickle")):
    stable_tree1 = {}
    for i, sub in tqdm(enumerate(allsubs1), total=len(allsubs1)):
        arg = np.where(allsubs1['id']==sub['id'])[0][0]
        branch = trees1[arg::len(allsubs1)]#[trees['lastid'] == target['id']]
        branch = branch[branch['take_score']>0]
        stable_tree1[sub['id']] = branch
    pklsave(stable_tree1,f"{database1}/stable_tree_raw.pickle", overwrite=True)