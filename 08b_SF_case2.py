...
print("ex: $ python3 08b_SF_case2.py [--kind d] [--type 0] (--draw 0) (--ncpu 48)")
import argparse
...
str1, str2, ids = cases2[pkind][ptype]
print(f"\n\n[{pkinds[pkind]}]: {str1}+{str2}: {len(ids)} founds")
if(len(ids)==0): exit()


if(not os.path.isdir(f"{database2}/photo/evolution/{pkind}")): os.makedirs(f"{database2}/photo/evolution/{pkind}")
if(not os.path.isdir(f"{database2}/photo/evolution/{pkind}/{ptype}")): os.makedirs(f"{database2}/photo/evolution/{pkind}/{ptype}")
path = f"{database2}/photo/evolution/{pkind}/{ptype}"

zoom = 1.5
dpi = 240
nmax = dpi**2 / 2
for targetid in ids:
    branch = stree2[targetid]
    ...
    for iout in tqdm(branch['timestep'][::-1], desc=f"[{targetid:07d}]"):
        ...
        l1=lax.plot(out2gyr(tmp['timestep'], snap2s), tmp['mdm_vir'], color='lime', label='DM', lw=2)
        l2=lax.plot(out2gyr(tmp['timestep'], snap2s), tmp['mstar_vir'], color='orange', label='Star', lw=2)
        l3=lax.plot(out2gyr(tmp['timestep'], snap2s), tmp['mcell_vir'], color='w', label='Gas', lw=2)
        l4=lax.plot(out2gyr(tmp['timestep'], snap2s), tmp['mcold_vir'], color='dodgerblue', label='Cold', lw=2)
        # Future Evolution
        tmp = branch[branch['timestep'] > iout]
        lax.plot(out2gyr(tmp['timestep'], snap2s), tmp['mdm_vir'], color='lime', ls=':', lw=0.5)
        # Current point
        tmp = branch[branch['timestep'] == iout]
        lax.scatter(out2gyr(tmp['timestep'], snap2s), tmp['mdm_vir'], s=100,ec='none',fc='lime', marker='*')
        lax.scatter(out2gyr(tmp['timestep'], snap2s), tmp['mstar_vir'], s=100,ec='none',fc='orange', marker='*')
        lax.scatter(out2gyr(tmp['timestep'], snap2s), tmp['mcell_vir'], s=100,ec='none',fc='w', marker='*')
        lax.scatter(out2gyr(tmp['timestep'], snap2s), tmp['mcold_vir'], s=100,ec='none',fc='dodgerblue', marker='*')
        # Star Formation
        tmp = branch[branch['timestep'] <= iout]
        l5=lax.fill_between(out2gyr(tmp['timestep'], snap2s), 0, np.cumsum(tmp['insitu_vir'][::-1])[::-1], ec='none', fc='darkorange', alpha=0.3, label='in-situ star')

        lax2 = lax.twinx(); lax2.set_zorder(-1); lax.patch.set_visible(False)
        stemcontainer = lax2.stem(out2gyr(branch['timestep'], snap2s), evolution['ndense'], linefmt='cyan', markerfmt='cyan', basefmt='none', label=r'$N_{dense}$')
